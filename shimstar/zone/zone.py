...
class Zone:
    def __init__(self, id):
        ...
        cursor.close()
        if self.typeZone == C_TYPEZONE_STATION:
            self.loadZoneStation()
    ...
    def loadZoneStation(self):
        query = "Select star022_inzone_star011 from star022_station where star022_zone_star011 = " + str(self.id)
        instanceDbConnector = shimDbConnector.getInstance()
        cursor = instanceDbConnector.getConnection().cursor()
        cursor.execute(query)
        result_set = cursor.fetchall()
        for row in result_set:
            self.exitZone = int(row[0])
        cursor.close()
